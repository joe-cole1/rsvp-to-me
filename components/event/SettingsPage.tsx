...
export function SettingsPage({ event, isOwner, themePresets = [] }: { event: EventInput; isOwner: boolean; themePresets?: DbThemePreset[] }) {
  ...
  // ── Theme Auto-Save ──
  const triggerSaveTheme = (newBase: BaseTheme, newFrom: string, newTo: string, newAccent: string, presetId?: string | null, newCardOpacity?: number) => {
    setSaveStatus("SAVING");
    setErr(null);
    startTransition(async () => {
      try {
        await saveEventTheme(event.id, newBase, newFrom, newTo, newAccent, presetId !== undefined ? presetId : themePresetId, newCardOpacity !== undefined ? newCardOpacity : cardOpacity);
        setSaveStatus("SAVED");
      } catch {
        setSaveStatus("ERROR");
                    ...
                    {hasChanged && (
                      <button
                        onClick={() => {
                          ...
                          triggerSaveTheme(savedBase, savedFrom, savedTo, savedAccent, undefined, savedOpacity);
                        }}
