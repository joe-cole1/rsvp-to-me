...
- **Orphaned `components/event/ThemePicker.tsx`**: Nothing imports this component anymore — theme editing moved into `components/event/settings-page/ThemePanel.tsx` when the cover-image theme button was removed. It has since drifted behind the live panel (no card opacity, font, or effect support). Delete it, or fold anything still useful into ThemePanel. _(Discovered during the [89d70d] rich/seasonal themes work; left out of scope to keep that PR focused.)_
