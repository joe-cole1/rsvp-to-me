...
#### STRIDE threat model — host-auth & invite-issuance flows (2026-07)

_Residual findings from a STRIDE threat model scoped to the host-authentication and invite-link-issuance flows. The high-value fixes — sign-in enumeration (**SEC-40**), shared-bucket sign-in lockout (**SEC-45**), invite-issuance attribution (**SEC-46**), and LOGIN-only initial-admin bootstrap (**SEC-48**) — shipped; see Completed Milestones. The Host-header redirect finding overlaps the audit's **SEC-41** and is tracked there. The items below remain deferred because each needs a design/UX/config decision or a schema migration._

- **[SEC-47] Co-host invitation grants a global `HOST` role** (Elevation of Privilege / scope): `app/auth/verify-cohost/route.ts:43-56` sets `User.role = "HOST"` globally when a co-host invite is accepted, granting app-wide host powers (create arbitrary events, `/dashboard`, host actions) rather than co-host rights scoped to the one event. _Fix_: keep `User.role` unchanged for pure co-hosts and derive host capabilities from `Event.hostId ∪ EventCoHost` membership (the `assertHostOrCohost` model already exists). _(STRIDE 2026-07.)_
- **[SEC-49] No authentication/audit trail for logins or invite issuance** (Repudiation): `Session` rows (`lib/session.ts:162-198`) store only `id/userId/expiresAt` — no IP, user-agent, or token-consumption record — so a compromised login is untraceable. _Fix_: add an `AuthEvent`/audit model capturing `getClientIp()` + UA at each verify/`createSession`. Requires a schema migration. _(STRIDE 2026-07.)_
- **[SEC-50] Magic-link token-invalidation churn enables targeted sign-in denial** (DoS): `createMagicLink` (`lib/auth.ts:48-51`) invalidates _all_ of a user's unused tokens on each issuance; an attacker who knows a victim's email can fire up to 5/10 min (the per-identifier cap) and invalidate the link the victim is trying to use. _Fix_: let multiple short-lived tokens coexist and burn only on use, or scope invalidation to the same request context. _(STRIDE 2026-07.)_
- **[SEC-51] Co-host invite mints a session from token possession alone** (Spoofing): `app/auth/verify-cohost/route.ts:29-56` creates/authenticates a session as the invited email's identity purely on possession of the 48h token, with no proof the clicker owns that inbox. _Fix_: require the recipient to authenticate via their own magic link first, then _consume_ the co-host token to attach the role. _(STRIDE 2026-07.)_
...
- [x] **Scope Next.js element lint rules away from React Email markup**: `emails/components/EmailLayout.tsx` intentionally emits an email-document `<head>`, and `emails/components/Hero.tsx` intentionally uses a raw `<img>` for email-client compatibility. Next's `@next/next/no-head-element` and `@next/next/no-img-element` rules are disabled only for `emails/**/*.tsx`; application pages retain both rules. _(Completed 2026-07.)_
...
- **[SEC-44] `decryptConfig` passes through legacy plaintext**: `lib/crypto.ts:30-32` returns the input unchanged when it contains no `:` separator, a deliberate backward-compat affordance for values stored before encryption was added. It means a stored secret that doesn't look like ciphertext is silently trusted as plaintext rather than rejected. Very low severity; revisit once all legacy `SystemConfig` secret rows are known to be encrypted, then remove the passthrough. _(Discovered during the [cd6748] OWASP audit.)_
- **[SEC-52] Magic-link raw token exposure — defence-in-depth** _(hardening; STRIDE 2026-07)_ (Spoofing): the raw login token travels in the URL and email body (`lib/auth.ts:64`), so any leak (forwarded email, SMTP/proxy log, shared device) yields a full host session. Already mitigated by `no-referrer`, 15-min TTL, single-use, and SHA-256-at-rest. Optional further hardening: bind the token to a device cookie set at request time, shorten the TTL to ~5 min, and email a "new sign-in" notice.
- **[SEC-53] Registration reveals account existence** _(needs UX decision; STRIDE 2026-07)_ (Information Disclosure): `registerHost` (`lib/auth.ts:237-241`) returns "An account with this email already exists" for known emails, allowing enumeration on the register endpoint (rate-limited 3/hr). Sibling to the fixed SEC-40 sign-in leak, but the register fix needs a product call on the UX (e.g. a uniform "if eligible, we've emailed you next steps"). Pairs well with **G-6** (Turnstile/CAPTCHA on auth forms).
- **[SEC-54] Post-login open-redirect allowlist** _(hardening; STRIDE 2026-07)_ (Tampering): the `redirect` query param flows into `${origin}${rawRedirect}` at `app/auth/verify/route.ts:81`. Already mitigated by `isSafeRedirect` (`lib/auth.ts:21-29`, double-checked at the verify route), which blocks `//`, `:`, and `/\`. Optional: replace the string check with an allowlist of known internal path prefixes. Distinct from the audit's **SEC-41** (Host-header origin).
- **[SEC-55] Invite / RSVP / event-password IP limiters share the loopback bucket without a trusted proxy** _(STRIDE 2026-07)_ (DoS): the SEC-45 fix skipped the shared-loopback IP tier for magic-link sign-in only. The same coarse behaviour remains for the other IP-keyed limiters (`inviteGuest`, `inviteFriendAsGuest`, `addRSVP`, event-password unlock — all via `getClientIp()`), but those sit behind host auth or a token and carry secondary per-user/per-token caps, so the availability impact is far lower. Revisit alongside **G-6**, or make `TRUSTED_IP_HEADER` a documented requirement for multi-tenant deploys (a startup warning when unset in production).
...
- [x] **Host-facing whole-party check-in**: Every linked RSVP status—including unanswered invitees, Can't make it, and awaiting-approval guests—can be checked in or undone by hosts, co-hosts, and admins from the guest list. PR #306 expanded the original Going/Maybe scope so door staff can handle real attendance without first rewriting RSVP status. Host-only search, Arrived/Not arrived filters, optimistic controls, event-time display, and party/people progress make the page usable at the door; other devices converge on refresh.
...
- [x] **[SEC-36] Host-supplied URL protocol validation**: `HttpUrlSchema` in `lib/schemas.ts` now gates every write path for the event virtual link (`CreateEventSchema`, `saveEventField`, `saveEventLocation`) and info-section `url` (`addInfoSection`/`updateInfoSection`) — only http(s) may be persisted, killing `javascript:`/`data:` URIs rendered into guest-facing `<a href>`. Scheme-less pastes ("zoom.us/j/…") are auto-prefixed with `https://` so the normal host workflow keeps working.
- [x] **[SEC-37] RSVP contact format validation**: `AddRsvpSchema.guestEmail` now requires a valid email (it's a `User` upsert key + mail recipient) and `guestPhone` the invite-flow phone shape (`GuestPhoneRegex`), blocking junk `User` identities and skewed `linkRsvpsToUser` matching.
- [x] **[SEC-38] Questionnaire answers validated**: answer values capped at 2000 chars (keys at 100) in both RSVP schemas, and `addRSVP`/`updateRSVP` now drop answer keys that don't belong to the event's own `rsvpFields` — a crafted call can no longer attach rows to another event's question ids.
- [x] **[SEC-41] Verify-route redirect origin**: `app/auth/verify/route.ts` now builds redirects from `NEXT_PUBLIC_APP_URL` (trailing slashes stripped); the `Host`/`x-forwarded-proto` derivation remains only as a fallback for installs that never set it, closing the Host-header link-poisoning vector behind non-pinning proxies.
- [x] **[SEC-42] RSVP existence oracle closed**: `deleteRsvpAsHost`/`approveRsvp`/`declineRsvp` now require a session before any lookup ("Unauthorized") and throw the same "Forbidden" for a missing id as for an unauthorized one (matching the info-section actions), so callers can't probe which RSVP ids exist.
- [x] **[SEC-56] Logged-in-guest private-event gate — resolved by SEC-34**: verified that `resolveEventAccess()` (`lib/eventAccess.ts`, extracted in the SEC-34 batch) already admits a logged-in guest via an RSVP lookup scoped to `{ userId: session.userId, eventId: event.id }` and reuses the row as `loggedInUserRsvp` — exactly the fix preserved from the deleted `feature/b7081d` branch. The re-verification concern is satisfied: the query is event-scoped, so a matching `userId` with no RSVP on _this_ event gets no row, no access, and no leaked fields. Covered by `tests/regression/sec-34-event-access-gates.test.ts` ("grants a logged-in guest who already has an RSVP on the event"). No code change needed.
- [x] **RSVP Flow confirmation race**: a guest submitting a new RSVP transiently saw "RSVP updated!" instead of "You're in!" — the Server Action's automatic RSC refresh re-requested the page with the new `?token=` URL, delivering `existingRsvp` to the still-mounted `RsvpFlow` and flipping its `isEdit` heading. A client-side `justCreated` flag now pins the create heading for the RSVP created in this mount. _(Found in the [89d70d] themes E2E audit.)_
- Regression tests: `tests/regression/sec-36-url-protocol-validation.test.ts`, `sec-37-rsvp-contact-format.test.ts`, `sec-38-answers-validation.test.ts`, `sec-41-verify-host-header-origin.test.ts`, `sec-42-rsvp-existence-oracle.test.ts`, plus a race-condition describe in `tests/components/RsvpFlow.test.tsx`. Docs: `docs/admin/configuration.md`, `docs/host/creating-an-event.md`, `docs/host/customizing-your-page.md`.

### OWASP Audit — Access Control & Data-Leak Fixes (Batch [cd6748])

- [x] **Event-page host-only data leak ([SEC-32])**: `app/e/[slug]/page.tsx` passed the full Prisma event object into the `<EventPage>` Client Component, so host/co-host **email addresses** and the full approved **guest list** (even when `guestListVis` hid it in the UI) were serialized into the RSC payload for any viewer. Added `lib/guestList.stripHostOnlyEventData` to null host/co-host email and empty the guest list for non-hosts when the list isn't public, applied before the client boundary.
- [x] **`GUESTS_ONLY` guest list not enforced ([SEC-33])**: `app/e/[slug]/guests/page.tsx` only redirected on `HOST_ONLY`, so a `GUESTS_ONLY` event's full guest list was reachable by any anonymous visitor at `/e/[slug]/guests`. Added `lib/guestList.canViewGuestListPage`, which admits hosts and the event's own guests (logged-in guest or valid token) but never an anonymous visitor.
- [x] **Email console fallback leaked credentials in production ([SEC-35])**: `lib/email.ts` `send()` logged the full rendered HTML (magic-link + RSVP edit tokens) and recipient to stdout whenever no transport was configured, ungated by environment. Now the full-payload dump is development-only; production logs a generic "no transport configured" warning with no PII or tokens.
- Regression tests: `tests/regression/sec-32-33-host-only-event-data-leak.test.ts`, `tests/regression/sec-35-email-console-fallback-prod-leak.test.ts`. Subsequent audit findings and their current completed/deferred status are recorded in the security sections above and below.

### v1.3.0 Pre-Release Security & Roadmap Hardening (Batch [019f6da1])

- [x] **[SEC-39] Activity-feed guest privacy**: RSVP creation, update, deletion, and invitation activity now follows the existing guest-list authorization rules. Restricted entries—including actor names and RSVP note/detail text—are removed on the server before the event object crosses the `EventPage` Client Component boundary. Existing non-host stripping for check-in, check-in undo, and walk-in activity remains intact.
- [x] **[SEC-48] LOGIN-only initial-admin bootstrap**: `INITIAL_ADMIN_EMAIL` promotion now runs only after successful verification of a `LOGIN` magic token. Ordinary session reads, dashboard/profile access, profile-change verification, and co-host invitation acceptance cannot promote; the configured-email and no-existing-admin checks remain required.
- [x] **[CLEAN-02] Shared authorization guards already centralized**: `lib/auth-guards.ts` is the source of truth for `assertHost`, `assertHostOrCohost`, and `assertAdmin`; the guest CSV and backup download routes already consume those shared guards, so the stale open cleanup item was closed without another refactor.
- Regression tests: `tests/regression/sec-39-activity-feed-guest-privacy.test.ts`, `tests/regression/sec-48-initial-admin-login-only.test.ts`. Docs: `docs/admin/admin.md`, `docs/admin/configuration.md`, `docs/admin/installation.md`, `docs/host/guest-list.md`, `docs/host/messaging-and-reminders.md`.

### Auth & Invite-Flow Hardening — STRIDE (Batch `feature/auth-invite-hardening`)

_High-value, low-risk fixes from the 2026-07 STRIDE threat model of the host-authentication and invite-link-issuance flows. Remaining findings are tracked as **SEC-47**, **SEC-49…SEC-55** (plus the audit's SEC-41) above._

- [x] **[SEC-40] Sign-in account enumeration**: `sendMagicLinkAction` (`app/actions/auth.ts`) now returns the same `{ success: true }` "check your inbox" response for unknown identifiers (sending nothing) instead of a distinct `auth_failed` state, closing the enumeration channel that survived SEC-14a. Resolves the deferred audit finding **SEC-40**.
- [x] **[SEC-45] Shared-bucket sign-in lockout**: the per-IP sign-in limiter now runs **only when `TRUSTED_IP_HEADER` is configured** — without a trusted proxy every request resolves to loopback, so the old `ip:127.0.0.1:magic-link` bucket was global and 20 attempts anywhere locked out all sign-ins. The per-identifier limiter (5 / 10 min) remains active; new `isTrustedIpConfigured()` in `lib/clientIp.ts`. Host registration keeps its global limiter to stay invite-code brute-force-resistant.
- [x] **[SEC-46] Invite issuance now attributed in the activity log**: `inviteGuest` (`app/actions/event/invites.ts`) was the only invite path writing no `ActivityEvent`; it now logs a `guest_invite` activity attributed to the acting host, matching `inviteFriendAsGuest`.
- Regression tests: `tests/regression/sec-40-45-signin-enumeration-shared-ip.test.ts`, `tests/regression/sec-46-invite-guest-activity-log.test.ts`. Docs: `docs/admin/configuration.md`, `docs/host/inviting-guests.md`.
