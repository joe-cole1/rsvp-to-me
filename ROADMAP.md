...
- **[SEC-40] Magic-link account enumeration**: `sendMagicLinkAction` (`app/actions/auth.ts:47-49`) returns a distinguishable `auth_failed` for unknown accounts vs. `success` for known ones, allowing account enumeration. Mitigated by the per-IP/per-identifier rate limits. _Fix_: return a uniform success response whether or not the identifier exists. Severity low. _(Discovered during the [cd6748] OWASP audit.)_
