...
- **[SEC-36] Host-supplied URLs rendered as links without protocol validation**: `virtualUrl` (`AddRsvpSchema`/settings — validated only as `z.string().max(1000)` in `lib/schemas.ts`) and info-section `url` (stored unvalidated in `app/actions/event/infoSections.ts`) are rendered into `<a href>` for guests (`components/event/event-page/LocationEdit.tsx:294`, `components/event/event-page/InfoSectionsBlock.tsx:163`). A host (anyone, under open registration) can set a `javascript:`/`data:` URI as the "join" link and a guest who clicks it executes it. React's dev-only warning on `javascript:` hrefs is incidental, not a control. _Fix_: validate `^https?://` server-side on both fields before persisting (reuse the existing invite-flow validation shape). Severity low-medium (requires a malicious/compromised host). _(Discovered during the [cd6748] OWASP audit.)_

- **[SEC-56] Logged-in-guest private-event gate bypass (unfinished, `feature/b7081d-guest-list-hostbar-ux`)**: `app/e/[slug]/page.tsx`'s PRIVATE-event gate currently admits only `isHost`, `isUnlocked` (password), or `hasValidToken` — a guest who is logged in and already has an RSVP for the event (matched via `session.userId`) but never unlocked via password/token is incorrectly blocked from their own event page. A branch has a fix: look up `loggedInUserRsvp` by `{ userId: session.userId, eventId: event.id }` and add `!isLoggedInGuest` to the gate condition, reusing the fetched row as `guestRsvp` instead of a second query. **Before reviving**: this touches the same gate logic as **SEC-34**, so re-verify it can't leak `loggedInUserRsvp` fields (or grant access) to a `session.userId` that matches on identity but isn't actually invited to *this* event — confirm the query is scoped correctly and no fields leak to a non-invited viewer. No PR was ever opened for it. _(Branch deleted 2026-07-12 after a stray-branch cleanup; diff preserved here.)_

#### STRIDE threat model — host-auth & invite-issuance flows (2026-07)

_Residual findings from a STRIDE threat model scoped to the host-authentication and invite-link-issuance flows. The high-value fixes — sign-in enumeration (**SEC-40**), shared-bucket sign-in lockout (**SEC-45**), and invite-issuance attribution (**SEC-46**) — shipped; see Completed Milestones. The Host-header redirect finding overlaps the audit's **SEC-41** and is tracked there. The items below were deferred because each needs a design/UX/config decision or a schema migration._
...
- **[SEC-48] Initial-admin auto-promotion reachable from any session, not just a verified login** (Elevation of Privilege): `promoteInitialAdmin` (`lib/admin-promotion.ts:12-19`) runs inside `getSession()` (`lib/session.ts:142`) and every verify path. While `INITIAL_ADMIN_EMAIL` is set and no admin exists yet, any session for a user whose email matches — including an account minted for an arbitrary email by the co-host or RSVP paths — is auto-promoted to `ADMIN`. _Fix_: gate promotion to the verified `LOGIN` token path only, or move it out-of-band to a CLI/seed step. Tightens the scope of SEC-32's guard. _(STRIDE 2026-07.)_
...
- **[SEC-37] `guestEmail` / `guestPhone` not format-validated in `AddRsvpSchema`** (`lib/schemas.ts`): both are accepted as length-capped strings only, then `guestEmail` becomes a `User` upsert key and a mail recipient in `addRSVP` (`app/actions/event/rsvp.ts`). An attacker can pollute the `User` table with junk identities and skew `linkRsvpsToUser` matching. _Fix_: add `.email()` and a phone regex — the host invite flow (`app/actions/event/invites.ts:56-64`) already validates both; reuse that shape. _(Discovered during the [cd6748] OWASP audit.)_
- **[SEC-38] RSVP questionnaire `answers` unvalidated** (`lib/schemas.ts` `AddRsvpSchema`/`UpdateRsvpSchema`): `z.record(z.string(), z.string())` puts no length cap on answer values (unbounded storage vs. the capped `note`/comment fields) and the map keys are written as `rsvpFieldId` in `addRSVP`/`updateRSVP` without verifying the field belongs to this event. _Fix_: cap value length (~2000) and validate each `rsvpFieldId` against the event's `rsvpFields`. _(Discovered during the [cd6748] OWASP audit.)_
...
- **[SEC-41] Magic-link verify builds redirect origin from the `Host` header**: `app/auth/verify/route.ts:13-15` derives `origin` from the request `Host`/`x-forwarded-proto` headers. Behind a proxy that does not pin `Host`, this is a redirect/link-poisoning vector. _Fix_: prefer `NEXT_PUBLIC_APP_URL` for the redirect base. Severity low. _(Discovered during the [cd6748] OWASP audit.)_
- **[SEC-42] Existence oracle on host RSVP actions**: `deleteRsvpAsHost`/`approveRsvp`/`declineRsvp` (`app/actions/event/rsvp.ts`) throw "Not found" _before_ the `assertHostOrCohost` authorization check, letting a caller distinguish existing from non-existing RSVP ids. Negligible in practice (ids are unguessable cuids) but the check order should be authorize-then-fetch. _(Discovered during the [cd6748] OWASP audit.)_
...
- Regression tests: `tests/regression/sec-32-33-host-only-event-data-leak.test.ts`, `tests/regression/sec-35-email-console-fallback-prod-leak.test.ts`. Remaining audit findings logged above as **SEC-36–SEC-44** (deferred).

### Auth & Invite-Flow Hardening — STRIDE (Batch `feature/auth-invite-hardening`)

_High-value, low-risk fixes from the 2026-07 STRIDE threat model of the host-authentication and invite-link-issuance flows. Remaining findings are tracked as **SEC-47…SEC-55** (plus the audit's SEC-41) above._
