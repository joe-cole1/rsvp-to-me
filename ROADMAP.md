...
- **Auth and Invite Hardening (`feature/auth-invite-hardening`)**: Prevent email enumeration on authentication and invitation verification endpoints, implement IP-based rate limiting for sensitive flows, and harden invite guest logs.
...
#### STRIDE threat model — auth & invite-link flows (2026-07)

_Residual findings from a STRIDE threat model scoped to the host-authentication and invite-link-issuance flows. The two highest-value, lowest-risk items (sign-in enumeration + shared-bucket lockout, and invite-issuance attribution) were fixed under **SEC-35 / SEC-36** — see Completed Milestones. The items below were deferred because each needs a design/UX/config decision or a schema migration; they should not be bundled into an unrelated PR._

- **[SEC-37] Host-header trust in magic-link / co-host verify redirect** (Tampering): `app/auth/verify/route.ts:11-14` and `app/auth/verify-cohost/route.ts:11-13` build the post-login redirect `origin` (the domain the freshly-set session cookie lands on) from the attacker-influenceable `Host` / `x-forwarded-proto` request headers. A self-hosted deploy behind a proxy that does not pin `Host` could be steered to an attacker origin. **Decision needed:** the app deliberately uses the `Host` header today so a single deployment serves multiple hostnames (see the intentional test `tests/auth/verify.test.ts:144` and the live `rsvp.thecolefam.com` host). Fix candidates: derive `origin` from `NEXT_PUBLIC_APP_URL` (already the source of truth in `createMagicLink`), or add a `Host` allowlist env var. Choose before implementing — it changes multi-domain behavior.
- **[SEC-38] Co-host invitation grants a global `HOST` role** (Elevation of Privilege / scope): `app/auth/verify-cohost/route.ts:43-56` sets `User.role = "HOST"` globally when a co-host invite is accepted, granting app-wide host powers (create arbitrary events, `/dashboard`, host actions) rather than co-host rights scoped to the one event. Fix: keep `User.role` unchanged for pure co-hosts and derive host capabilities from `Event.hostId ∪ EventCoHost` membership (the `assertHostOrCohost` model already exists). Needs an audit of everywhere `role === "HOST"` is assumed.
- **[SEC-39] Initial-admin auto-promotion reachable from any session, not just a verified login** (Elevation of Privilege): `promoteInitialAdmin` (`lib/admin-promotion.ts:12-19`) runs inside `getSession()` (`lib/session.ts:142`) and every verify path. While `INITIAL_ADMIN_EMAIL` is set and no admin exists yet, any session for a user whose email matches — including an account minted for an arbitrary email by the co-host or RSVP paths — is auto-promoted to `ADMIN`. Fix: gate promotion to the verified `LOGIN` token path only, or move it out-of-band to a CLI/seed step. (SEC-32 consolidated the guard; this tightens _which entry points_ may trigger it.)
- **[SEC-40] No authentication/audit trail for logins or invite issuance** (Repudiation): `Session` rows (`lib/session.ts:162-198`) store only `id/userId/expiresAt` — no IP, user-agent, or token-consumption record — so a compromised login is untraceable. Add an `AuthEvent`/audit model capturing `getClientIp()` + UA at each verify/`createSession`. Requires a schema migration.
- **[SEC-41] Magic-link token-invalidation churn enables targeted sign-in denial** (DoS): `createMagicLink` (`lib/auth.ts:48-51`) invalidates _all_ of a user's unused tokens on each issuance; an attacker who knows a victim's email can fire up to 5/10 min (the per-identifier cap) and invalidate the link the victim is trying to use. Fix: let multiple short-lived tokens coexist and burn only on use, or scope invalidation to the same request context.
- **[SEC-42] Co-host invite mints a session from token possession alone** (Spoofing): `app/auth/verify-cohost/route.ts:29-56` creates/authenticates a session as the invited email's identity purely on possession of the 48h token, with no proof the clicker owns that inbox. Fix: require the recipient to authenticate via their own magic link first, then _consume_ the co-host token to attach the role, rather than minting a session directly from the invite token.
...
- **[SEC-43] Magic-link raw token exposure — defence-in-depth** _(hardening; STRIDE 2026-07)_ (Spoofing): the raw login token travels in the URL and email body (`lib/auth.ts:64`), so any leak (forwarded email, SMTP/proxy log, shared device, browser history) yields a full host session. Already mitigated by `no-referrer`, 15-min TTL, single-use, and SHA-256-at-rest. Optional further hardening: bind the token to a device cookie set at request time and require it at verify, shorten the TTL to ~5 min, and email a "new sign-in" notice. Low priority given existing controls.
- **[SEC-44] Registration reveals account existence** _(needs UX decision; STRIDE 2026-07)_ (Information Disclosure): `registerHost` (`lib/auth.ts:237-241`) returns "An account with this email already exists" for known emails, allowing enumeration on the register endpoint (rate-limited 3/hr). Sibling to the fixed SEC-35 sign-in leak, but the fix needs a product call on the register UX (e.g. return a uniform "if eligible, we've emailed you next steps" and deliver the differentiator by email). Pairs well with **G-6** (Turnstile/CAPTCHA on auth forms).
- **[SEC-45] Post-login open-redirect allowlist** _(hardening; STRIDE 2026-07)_ (Tampering): the `redirect` query param flows into `${origin}${rawRedirect}` at `app/auth/verify/route.ts:81`. Already mitigated by `isSafeRedirect` (`lib/auth.ts:21-29`, double-checked at the verify route), which blocks `//`, `:`, and `/\`. Optional: replace the string check with an allowlist of known internal path prefixes.
- **[SEC-46] Invite / RSVP / event-password IP limiters share the loopback bucket without a trusted proxy** _(STRIDE 2026-07)_ (DoS): the SEC-35 fix skipped the shared-loopback IP tier for magic-link sign-in only. The same coarse behaviour remains for the other IP-keyed limiters (`inviteGuest`, `inviteFriendAsGuest`, `addRSVP`, event-password unlock — all via `getClientIp()`), but those sit behind host auth or a token and carry secondary per-user/per-token caps, so the availability impact is far lower. Revisit alongside **G-6** or make `TRUSTED_IP_HEADER` a documented requirement for multi-tenant deploys (a startup warning when unset in production).
...
_The high-value, low-risk fixes bundled from the 2026-07 STRIDE threat model of the host-authentication and invite-link-issuance flows. Remaining findings (SEC-37 … SEC-46) are tracked under High/Medium/Low priority above._

- [x] **[SEC-35] Sign-in account enumeration + shared-bucket lockout**: `sendMagicLinkAction` (`app/actions/auth.ts`) now returns the same `{ success: true }` "check your inbox" response for unknown identifiers (sending nothing) instead of a distinct `auth_failed` state, closing the user-enumeration channel that survived SEC-14a. Separately, the per-IP sign-in limiter now runs **only when `TRUSTED_IP_HEADER` is configured** — without a trusted proxy every request resolves to loopback, so the old `ip:127.0.0.1:magic-link` bucket was global and 20 attempts anywhere locked out all sign-ins; the per-identifier limiter (5 / 10 min) remains the active protection in that mode. Added `isTrustedIpConfigured()` to `lib/clientIp.ts`; host registration keeps its global limiter to stay invite-code brute-force-resistant. Regression: `tests/regression/sec-35-signin-enumeration-shared-ip.test.ts`; docs updated in `docs/admin/configuration.md`.
- [x] **[SEC-36] Invite issuance now attributed in the activity log**: `inviteGuest` (`app/actions/event/invites.ts`) was the only invite path writing no `ActivityEvent` (the guest-facing `inviteFriendAsGuest` already logged), leaving host/co-host invite blasts unattributable. It now logs a `guest_invite` activity attributed to the acting host on success, fire-and-forget with `logSafe` (matching `addRSVP`). Regression: `tests/regression/sec-36-invite-guest-activity-log.test.ts`; docs updated in `docs/host/inviting-guests.md`.
